...
## Developed initially by Ardiansyah (UNSOED), http://ardiansyah.net, main email : ardi.plj@gmail.com , cc : ard@unsoed.ac.id
...
##Constant for fourier Ts (Ts is calculated, not input. This fourier only for comparison)
...
class atmospher:
    ...
    def raa(self,ha, u, hc, T0, Ta):  ## Boundary layer resistance, Tsurf = T0(vegetated) or Ts (bare soil)
        pm = 0
        ph = 0
        d  = 0.778*hc
        zm = 0.13*hc
        ...
        for i in range(a.size):
            ...
            sp = -Vk*ha*gr*K_h0*(T0-Ta)/((rho_a*Cp)*(Ta+273)*np.power(ustar,3)) ##Soil physic with basic- 12.14, stability parameter
#            if sp > 0:
#                ph = 4.7*sp
#                pm = ph
#            else:
...
#                pm = 0.6*ph
            # sp in numpy array (not single value), change the code 
            ph = np.where(sp>0, 4.7*sp, -2*np.log((1+np.sqrt(1-16*sp))/2))
            pm = np.where(sp>0, ph, 0.6*ph)
        return raa
    ...
    def H(self, T0, Ta):
        H = (rho_a * Cp) * (T0 - Ta)/self.raa(ha, u, hc, T0, Ta)
        return H
...
if __name__ == '__main__':  ## Run simulation as standalone program
    ...
    ## Penman-Monteith (single-layer) procedure, try without looping
    e_sat, de_sat = SVP(Ta)   
    e_act = AVP(Ta, Twb, alt)
    RH = e_act/e_sat  
...
###Update all node of T and p in soil 
    ...
    # 1-Layer Result
    fig1, ax = plt.subplots(3, 1, sharex='col', sharey='row')
    ax[0].plot(time, ETo, 'g-', label='ETo')
    ax[0].set_title('Penman-Monteith Evapotranspiration')
    #ax[0].set_xlabel('Time (24 hr decimal)')
    ax[0].set_ylabel('ETo ($W/m^2$)')
    ax[0].legend(loc='upper left', shadow=False, frameon=False)

    ax[1].plot(time, Rn, 'r-', label='Net Radiation')
    ax[1].plot(time, rad, 'b-', label='Global Rad.')
    ax[1].plot(time, Lw, 'g-', label='Net Longwave Rad.')
    ax[1].plot(time, Go, 'g--', label='Ground Heat Flux')
    #ax[1].set_xlabel('Time (24 hr decimal)')
    ax[1].set_ylabel('Radiation ($W/m^2$)')
    ax[1].legend(loc='upper left', shadow=False, frameon=False)

    ax[2].plot(time, Ta, 'r-', label='Air Temperature')
    secax = ax[2].twinx() # seondary yaxis
    secax.plot(time, RH, 'b-', label='Relative Humidity')
    secax.set_ylim([0, 1])
    secax.set_ylabel('Relative Humidity (x 100%)')
    secax.legend(loc='center right', shadow=False, frameon=False)
    ax[2].set_xlabel('Time (24 hr decimal)')
    ax[2].set_ylabel('Radiation ($W/m^2$)')
    ax[2].legend(loc='center left', shadow=False, frameon=False)

    plt.show()

    ## Double-Layer procedure, try without looping
    # Calculate energy balance in canopy surface and soil surface
    Rn_s = s.soil_Rn(Sw_in, Lw_in, LAI, alb_s, eps_s, Ts, Ta)
    Rn_c = p.canopy_Rn(Sw_in, Lw_in, LAI, alb_c, eps_c, Tc, Ta)
    H_s = s.soil_H(Ts, T0)
    H_c = p.canopy_H(Tc, T0) 
    H_a = a.H(T0, Ta) 
    LE_s = s.soil_LE(p_s, Ts, e_o)
    ...
    plt.figure(2)
    plt.subplot(211)
    plt.plot(time, Rn_s, 'r-', label='Sol Net Radiation')
    plt.plot(time, H_s, 'r-', label='Soil Sensible Heat Flux')
    plt.plot(time, LE_s, 'r-', label='Soil Evaporation')
    plt.plot(time, G_s, 'r-', label='Soil Ground Heat Flux')
    plt.xlabel('Time (24 hr decimal)')
    plt.ylabel('Energy ($W/m^2$)')
    plt.legend(loc='upper left', shadow=False, frameon=False)

    plt.subplot(221)
    plt.plot(time, Rn_c, 'r-', label='Canopy Net Radiation')
    plt.plot(time, H_a, 'r-', label='Canopy Sensible Heat Flux')
    plt.plot(time, LE_c, 'r-', label='Canopy Trnspiration')
    plt.plot(time, H_c, 'r-', label='Canopy Heat Flux')
    plt.xlabel('Time (24 hr decimal)')
    plt.ylabel('Energy ($W/m^2$)')
    plt.legend(loc='upper left', shadow=False, frameon=False)
