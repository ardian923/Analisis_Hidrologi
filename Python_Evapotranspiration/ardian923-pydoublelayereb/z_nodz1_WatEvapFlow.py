...
#dz1 = rs1.cell_value(43, 2) #hide if no dry layer
...
alt    = rs1.cell_value(12, 2)  ## altitude, ava. in EB prog

def geoGrid(depth, n):  ## Geometric grid with dry layer
    m = n - 2
    z = np.zeros(n + 1)
    sc = 0
...
def linGrid(depth, n):  ## Linear grid with dry layer
    m = n - 2
    z = np.zeros(n + 1)
    dz = depth / m
    z[0] = z[1] = 0
...
if __name__ == '__main__':  ## Run as standalone program
    ## Flux parameter for constant simulation put in main program, not executed if import as modul
    ...
    m = n - 2
    # z = geoGrid(depth, n) ## geometric grid
    z = linGrid(depth, n)  ## linear grid
